...
- **微信 docx 预览不支持制表位**：Tab/制表位方案在 Word/WPS/LibreOffice 里都对，但微信内置 docx 预览器会把 tab 当普通空格——**列对齐只有无框线表格这一条可靠路径**（不是渲染偏好，是硬性约束）。
- **默认段落样式可能是两端对齐（jc=both）**：表格单元格段落继承它后，长选项格内折行时首行会被拉伸出大空隙（单行时看不出来，属潜伏雷）。用 `post_save_patch.py --left-align-marker` 强制 jc=left。
- **instrText 拍平**：editor_sdk 保存时可能把域指令（HYPERLINK 等）变成可见文字，出现在非目标区；必须全文档扫描修复，并如实报告该回归。
...
- **核验脚本的假阳性**：多套卷子并存时题号重复（多份卷都有"23. A. …"），按题号匹配会误报折行/错位——核验必须带上下文或按唯一的选项文本定位。
- **WPS 特有属性**：`firstLineChars="200"`（按字符数缩进）在 Word 与 LibreOffice 中解释不同（差约 20pt），会造成跨平台 tab 起点漂移；表格方案天然免疫，但清理残留时要记得它。
- **AI 前序修改的残留物**：接手别人（或别的 AI）改过的文档时，重点扫：改色段落标记（如蓝色 `0000FF`）、被缩小的字号、残缺题号（"25A."）、网页水印混入的 2pt 乱码 run。
