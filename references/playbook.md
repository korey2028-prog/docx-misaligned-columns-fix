...
## 阶段 4：保存后 XML 补丁（已知副作用 + 前序修改残留）

跑 `scripts/post_save_patch.py <成品.docx> --cantsplit-marker "<表格特征文本>" --left-align-marker "<表格特征文本>"`，或手动：
...
3. **单元格强制左对齐**：给目标表格所有单元格段落设 `jc=left`。原因：试卷类文档默认段落样式常为两端对齐（实测本文档默认样式 jc=both），继承进单元格后，长选项格内折行时首行被拉伸。补丁前先查 styles.xml 的默认段落样式确认是否中招。
4. 补丁前自动备份；补丁后必须重跑阶段 5。
5. **接手"别的 AI 改过的文档"时的额外扫描**：残留的段落标记着色（如 0000FF）、WPS 的 `firstLineChars` 属性残留、被调小的字号（如 12pt→10.5pt）、混入的网页水印 2pt 乱码 run——逐一报告，由用户决定是否恢复。
...
4. **视觉验收（真实渲染，macOS 有 LibreOffice 时必做）**：

   ```bash
   # 沙箱环境须用独立用户配置目录，否则 LibreOffice 写 ~/Library 会被拦截
   soffice -env:UserInstallation=file:///tmp/lo_profile --headless \
     --convert-to pdf --outdir /tmp/render_check 成品.docx
   ```

   用 pymupdf（`pip install pymupdf`）做三级检查：
   a. **文字层**：遍历页面 get_text()，确认①目标文本所在页；②全 PDF 无域指令泄露（搜 `HYPERLINK` 等，应为 0 页）；③每题四个选项在同一页文本里。
   b. **几何层**：`page.get_text('words')` 按 y0 分行，统计各选项列（A./B./C./D.）的 x0——**range 必须为 0**；顺带确认表格跨页时是"整行流入下一页"而非行内拆分（验证 cantSplit 生效）。
   c. **目检层**：`page.get_pixmap(dpi=110).save(png)` 后直接查看图片；有原件 PDF 时渲染同区域页面做前后对照图。
   无法渲染时退级处理：明确标注"尚未完成视觉验收"，请用户在预览中翻查目标区及**后续页面**。
   ⚠️ **分页位移必须量化报告**：表格比原"折行硬塞"排版可能略高，导致后文整体后移 N 页（实测 20 题块 +1 页、总页数 416→417）。对比新旧 PDF 总页数，并定位第一个分歧页，确认纯位移、无内容增删。
   注：LibreOffice 与 Microsoft Word 渲染引擎不同，但 fixed 布局 + 显式列宽 + 实测字体的组合是确定性渲染；有条件时请用户在 Word/WPS 里终审一眼。
...
## 附录：批量阶段的"接力"场景（豆包对话实例）

本文档 skill 的真实来历：用户先让豆包做同一份试卷（2004–2026 天津卷）的完形对齐，豆包迭代了 5 轮把全部 26 节 520 行选项改成"手动题号 + Tab + 实测制表位 + 悬挂缩进 + 强制左对齐"，但最终卡在**微信 docx 预览不支持制表位**——它给出了"改用无边框表格"的结论却没有落地。接力要点：

1. **以 AI 前序产物为底稿**：豆包版每行格式已统一（`16. A. xxx<TAB>B. xxx<TAB>...`），按 Tab 切分即得五列内容，免去解析自动编号。
2. **审计前序产物而非盲信**：用本 playbook 的检查清单扫残留（首行缩进属性、着色标记、字号变动、水印乱码），逐项报告。
3. **两代方案的本质差异**：Tab 方案依赖渲染器正确实现制表位（Word/WPS/LO 都行，微信预览不行）；表格方案把对齐编码进文档结构，所有渲染器必须遵守。跨渲染器分发的文档一律用表格。
